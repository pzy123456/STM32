...
void initTIM4OC3(u16 duty)
{
	...
	TIM_OC3Init(TIM4, &TIM_OCInitStructure); 					  //生效初始化设置

	TIM_OC3PreloadConfig(TIM4, TIM_OCPreload_Enable);			  //使能输出比较预装载，即更改完输入捕获寄存器的值要等到更新事件发生才生效,该行加不加无所谓
}

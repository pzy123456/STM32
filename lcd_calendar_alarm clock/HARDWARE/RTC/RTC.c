 ...
 * 返回值：None
 */
static void NVIC_RTC_Config(void)
{
    NVIC_InitTypeDef NVIC_InitStructure;                                       //定义NVIC初始化结构体

    NVIC_InitStructure.NVIC_IRQChannel = RTC_IRQn;                         //指定配置的中断
...
u8 initRTC(void)
{
    ...
    if(BKP_ReadBackupRegister(BKP_DR1) != 0x6060)                               //判断是否为第一次使用RTC
    {
        ...
        BKP_WriteBackupRegister(BKP_DR1, 0x6060);                               //写上非第一标识
...
void showCalendar(Calendar_Structure* pcalendar)
{
    ...
    /*星期对应字符串*/
    u8* weekstr[7] = {"Sun ", "Mon ", "Tues", "Wed ", "Thur", "Fri ", "Satu"};
...
//    splitNumber(second,pcalendar->Second,sizeof(second),'0');
    sprintf(year, "%04d", pcalendar->Year);

    /*第一行显示 年/月/日 e.g.:2018/03/27*/
    LCD_ShowString(30, 150, 32, 16, 16, (u8 *)year);

    LCD_ShowChar(62, 150, '/', 16, 0);
    sprintf(month, "%02d", pcalendar->Month);
    LCD_ShowString(70, 150, 16, 16, 16, (u8 *)month);

    LCD_ShowChar(86, 150, '/', 16, 0);
    sprintf(day, "%02d", pcalendar->Day);
    LCD_ShowString(94, 150, 16, 16, 16, (u8 *)day);


    /*第二行显示 时:分:秒 e.g.:23:05:23*/
    sprintf(hour, "%02d", pcalendar->Hour);
    LCD_ShowString(30, 170, 16, 16, 16, (u8 *)hour);

    LCD_ShowChar(46, 170, ':', 16, 0);
    sprintf(minute, "%02d", pcalendar->Minute);
    LCD_ShowString(54, 170, 16, 16, 16, (u8 *)minute);

    LCD_ShowChar(70, 170, ':', 16, 0);
    sprintf(second, "%02d", pcalendar->Second);
    LCD_ShowString(78, 170, 16, 16, 16, (u8 *)second);


    /*第三行显示星期-3是因为算法原因没找到 e.g.:Tues*/
    LCD_ShowString(30, 190, 64, 16, 16, weekstr[(pcalendar->Week)]);


    /* 下面显示闹钟时间 */
    /*第四行显示闹钟时间以及是否开启 e.g.:08:30   on*/
    sprintf(alarm_hour, "%02d", pcalendar->AlarmHour);
    LCD_ShowString(30, 210, 64, 16, 16, (u8 *)alarm_hour);

    LCD_ShowChar(46, 210, ':', 16,0);
    
    sprintf(alarm_minute, "%02d", pcalendar->AlarmMinute);
    LCD_ShowString(54, 210, 64, 16, 16, (u8 *)alarm_minute);

    if(pcalendar->AlarmEN == ALARM_ON)
    {
        LCD_ShowString(78,210,64,16,16,"on ");
    }
    else
    {
        LCD_ShowString(78,210,64,16,16,"off");
    }
}
...
void configMode(CONFIGMODE_ENUM mode, Calendar_Structure* pcalendar)
{
    ...
    {
        printf("\r\n++++++++++++++++++++Alarm Settings++++++++++++++++++++++++++++++");
        printf("\r\n Please input new Alarm, e.g.:1256");

...
u8 initRTC(void)
{
    ...
    if(BKP_ReadBackupRegister(BKP_DR1) != 0x4040)                               //判断是否为第一次使用RTC
    {
        ...
        BKP_WriteBackupRegister(BKP_DR1, 0x4040);                               //写上非第一标识
...
//    splitNumber(second,pcalendar->Second,sizeof(second),'0');
    ...
    /*第二行显示 时:分:秒 e.g.:23:05:23*/
      ...
      /*第三行显示星期-3是因为算法原因没找到 e.g.:Tues*/
        LCD_ShowString(30,190,64,16,16,weekstr[(pcalendar->Week)-3]);
}
